...
//_\SV
  ...
  //`include "spi_riscv_if.v"  // new added 
//_\SV
   ...
   module rv32i_core (
    ...
    input  wire         tx_irq,
    input  wire         rx_irq,
    output wire [1:0]  spi_reg_addr,
    output wire         spi_reg_we,
    output wire         spi_reg_re,
    output wire [31:0] spi_reg_wdata,
    input  wire [31:0] spi_reg_rdata,
    input  wire         spi_done
);

wire [31:0] mem_i_rdata = i_rdata;
wire        mem_i_ready = i_ready;
wire [31:0] mem_d_rdata = d_rdata;
wire        mem_d_ready = d_ready;

//_\SV
      // Layer 2 — intentionally empty. mem_i_rdata / mem_i_ready /
   // mem_d_rdata / mem_d_ready (QSPI), reg_rdata / tx_irq / rx_irq
   // (UART), and spi_reg_rdata / spi_done (SPI) are all supplied
   // externally, by rv32i_core.sv, before it `include`s the compiled
   // output of this file's \TLV block.
`include "top_gen.sv" //_\TLV
   ...
   // ----------------------------------------------------------------
   // MEMORY-ACCESS FSM — 14 states now (was 10). $state widened 2->4 bits.
   ...
   // UART_WRITE_BUSY=9  // below added new 
   // SPI_READ=10, SPI_READ_BUSY=11, SPI_WRITE=12, SPI_WRITE_BUSY=13
   // ----------------------------------------------------------------
   ...
   assign L0_next_state_a0[3:0] =                                                   // CHANGED — full rewrite for 10 states
      ...
      (L0_state_a0 == 4'd1) ? 4'd2 :                                                    // FETCH_REQ: unconditional
            (L0_state_a0 == 4'd2) ? (!mem_i_ready                       ? 4'd2 :
                           (L0_is_load_a0  && L0_peri_sel_a0==2'b00)    ? 4'd3 :
                           (L0_is_store_a0 && L0_peri_sel_a0==2'b00)    ? 4'd4 :
                           (L0_is_load_a0  && L0_peri_sel_a0==2'b01)    ? 4'd6 :
                           (L0_is_store_a0 && L0_peri_sel_a0==2'b01)    ? 4'd8 :
                           (L0_is_load_a0  && L0_peri_sel_a0==2'b10)    ? 4'd10 :     // NEW — SPI_READ
                           (L0_is_store_a0 && L0_peri_sel_a0==2'b10)    ? 4'd12 :     // NEW — SPI_WRITE
      ...
      (L0_state_a0 == 4'd10) ? 4'd11 :                                                   // NEW — SPI_READ: unconditional
      (L0_state_a0 == 4'd11) ? 4'd1 :                                                    // NEW — SPI_READ_BUSY: unconditional
      (L0_state_a0 == 4'd12) ? 4'd13 :                                                   // NEW — SPI_WRITE: unconditional
      (L0_state_a0 == 4'd13) ? (spi_done ? 4'd1 : 4'd13) :                               // NEW — SPI_WRITE_BUSY: wait for spi_done
                          4'd0;
   ...
   assign L0_commit_a0 = ((L0_state_a0 == 4'd2) && mem_i_ready && !L0_is_mem_op_a0) ||
             ((L0_state_a0 == 4'd5) && mem_d_ready)                  ||
             (L0_state_a0 == 4'd7)                                   ||
             ((L0_state_a0 == 4'd9) && tx_irq)                       ||
             (L0_state_a0 == 4'd11)                                  ||           // NEW — SPI read done
             ((L0_state_a0 == 4'd13) && spi_done);                                // NEW — SPI write done
   ...
   assign L0_reg_wdata_a0[31:0] = L0_src2_value_a0;                    // NEW — same source as a QSPI store's $d_wdata
     ...
     // ---- SPI wiring — NEW ----
   assign L0_spi_reg_addr_a0[1:0]  = L0_ld_st_addr_a0[3:2];             // NEW — selects DATA/STATUS/CTRL/CONFIG
   assign L0_spi_reg_we_a0         = (L0_state_a0 == 4'd12);            // NEW — pulses during SPI_WRITE
   assign L0_spi_reg_re_a0         = (L0_state_a0 == 4'd10);            // NEW — pulses during SPI_READ
   assign L0_spi_reg_wdata_a0[31:0] = L0_src2_value_a0;                 // NEW — same source pattern as UART/QSPI 
   ...
   assign L0_result_a0[31:0] = L0_is_addi_a0 ? L0_src1_value_a0 + L0_imm_a0 :
                   ...
                   (L0_is_lw_a0 && (L0_peri_sel_a0 == 2'b10)) ? spi_reg_rdata :           // NEW — SPI word load
...
/*SV_plus*/
   ...
   assign reg_wdata = L0_reg_wdata_a0;
      // SPI bridges — NEW. These become rv32i_core.sv's new output ports.
   assign spi_reg_addr  = L0_spi_reg_addr_a0;
   assign spi_reg_we    = L0_spi_reg_we_a0;
   assign spi_reg_re    = L0_spi_reg_re_a0;
   assign spi_reg_wdata = L0_spi_reg_wdata_a0;
//_\SV
   endmodule
